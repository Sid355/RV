...
####     Kmeans CLUSTER bna rha tha.... 



stock_file='OLA.txt'

isin= 'NSE_EQ%7CINE0LXG01040'


def p():   ## 2 din continuously x% niche geya to next day kitna % change ho rha
    ml=open(stock_file).read()
    print("NHPC")
    pl=ml.split('\n')[:-1]
...
def d():                ### it will write historical 1 min data in stock_file
    import requests
    ...
    open(stock_file,'w').write(response.text)

def tkp():              ###  it will return 1 min intraday data (current day)
    import requests
